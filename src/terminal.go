...
func (t *Terminal) printHighlighted(result *Result, attr tui.Attr, col1 tui.ColorIndex, col2 tui.ColorIndex, current bool) {
	item := result.item
	...
	offsets := result.colorOffsets(charOffsets, t.theme, col2.Pair(), attr, current)
	...
	for _, offset := range offsets {
		...
		if b < e {
			...
			t.window.CPrintPair(offset.color, offset.attr, substr)
		}
	...
	extractColor(t.previewer.text, nil, func(str string, ansi *ansiState) bool {
		...
		if !t.preview.wrap {
			...
			for i, line := range lines {
				...
				trimmed, _ := t.trimRight([]rune(line), limit)
