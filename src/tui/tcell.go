...
func (i ColorIndex) style() tcell.Style {
	return i.Pair().style()
}
...
func (w *TcellWindow) Print(text string) {
	w.printString(text, ColDefault.Pair(), 0)
}
...
func (w *TcellWindow) printString(text string, pair ColorPair, a Attr) {
	...
	if w.color {
		...
		style = ColDefault.style().
			Reverse(a&Attr(tcell.AttrReverse) != 0 || pair.index == ColCurrent || pair.index == ColCurrentMatch).
			Underline(a&Attr(tcell.AttrUnderline) != 0 || pair.index == ColMatch || pair.index == ColCurrentMatch)
	}
	style = style.
...
func (w *TcellWindow) CPrint(color ColorIndex, a Attr, text string) {
	w.printString(text, color.Pair(), a)
}

func (w *TcellWindow) CPrintPair(pair ColorPair, attr Attr, text string) {
...
func (w *TcellWindow) Fill(str string) bool {
	return w.fillString(str, ColDefault.Pair(), 0)
}

func (w *TcellWindow) CFill(fg Color, bg Color, a Attr, str string) bool {
	return w.fillString(str, ColorPair{fg, bg, ColUser}, a)
}
