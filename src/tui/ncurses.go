...
var (
	_screen   *C.SCREEN
	_colorMap map[int]ColorIndex
	_colorFn  func(ColorIndex, Attr) (C.short, C.int)
)

func init() {
	_colorMap = make(map[int]ColorIndex)
...
func initPairs(theme *ColorTheme) {
	...
	for idx := 0; idx < len(Pallete); idx++ {
		pair := Pallete[idx]
		C.init_pair(C.short(idx), C.short(pair.Fg()), C.short(pair.Bg()))
	}
}
...
func attrColored(color ColorIndex, a Attr) (C.short, C.int) {
	return C.short(color), C.int(a)
}

func attrMono(color ColorIndex, a Attr) (C.short, C.int) {
...
func (w *CursesWindow) CPrint(color ColorIndex, attr Attr, text string) {
...
func (w *CursesWindow) CPrintPair(pair ColorPair, attr Attr, text string) {
	w.CPrint(pair.Index(), attr, text)
}
...
func (w *CursesWindow) CFill(fg Color, bg Color, attr Attr, str string) bool {
	index := ColorPair{fg, bg, ColUser}.Index()
...
func (p ColorPair) Index() ColorIndex {
	if p.index != ColUser {
		return p.index
	}
	...
	id := ColorIndex(len(_colorMap) + int(ColUser))
