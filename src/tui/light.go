...
func (r *LightRenderer) getch(nonblock bool) int {
	b := make([]byte, 1)
	syscall.SetNonblock(int(r.ttyin.Fd()), nonblock)
...
func (w *LightWindow) Move(y int, x int) {
	...
	y += w.Top()
	if y > 0 {
		csi(fmt.Sprintf("%dB", y))
	}
	x += w.Left()
	if x > 0 {
		csi(fmt.Sprintf("%dC", x))
	}
}
...
func csiColor(fg Color, bg Color, attr Attr) bool {
	codes := append(attrCodes(attr), colorCodes(fg, bg)...)
	csi(";" + strings.Join(codes, ";") + "m")
	return len(codes) > 0
}
...
func (w *LightWindow) CPrint(color ColorIndex, attr Attr, text string) {
	if !w.colored {
		attr = attrFor(color, attr)
		w.CPrint2(colDefault, colDefault, attr, text)
		return
	}
	pair := color.Pair()
	w.CPrint2(pair.Fg(), pair.Bg(), attr, text)
}

func (w *LightWindow) CPrint2(fg Color, bg Color, attr Attr, text string) {
	if csiColor(fg, bg, attr) {
		defer csi("m")
	}
	stderr(text)
}

func (w *LightWindow) CPrintPair(pair ColorPair, attr Attr, text string) {
	if !w.colored {
		csiColor(colDefault, colDefault, attrFor(pair.index, attr))
	} else {
		csiColor(pair.Fg(), pair.Bg(), attr)
	}
	stderr(text)
	csi("m")
}
...
func (w *LightWindow) setBg() {
	if w.bg != colDefault {
		csiColor(colDefault, w.bg, AttrRegular)
	}
}
...
func (w *LightWindow) CFill(fg Color, bg Color, attr Attr, text string) bool {
	...
	if csiColor(fg, bg, attr) {
		return w.fill(text, func() { csiColor(fg, bg, attr) })
		defer csi("m")
	}
	return w.fill(text, w.setBg)
}
